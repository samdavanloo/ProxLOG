...
% ISTA method, admm as proximal operator solver
%  Vant veer breast cancer dataset (295 samples,78 meta)
%% Setting
currentFile = sprintf('./Results/cancer_ISTA.mat');
lambda = 1e-3;
rho = 100;
step_size = 0.001; %step size of PGM
crt = 1e-8;
ite_max = 3000; % iteration for PGM
ite_ADMM = 1500; % iteration for inner ADMM
...
for ite = 1:ite_max
    ...
    prox = theta - step_size * grad;

    theta_pre = theta;

    [theta, P_LOG(ite)] = ADMM_L2(prox, G_idx, w, lambda * step_size, rho, crt, ite_ADMM);
...
save(currentFile, 'f', 'theta', 'time_CPU', 'time_real')
